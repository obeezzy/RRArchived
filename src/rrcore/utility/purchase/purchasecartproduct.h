...
struct PurchaseCartProduct
{
    ...
    explicit PurchaseCartProduct(
        const QString& product, const Stock::ProductCategory& category,
        const QString& description, const QUrl& imageUrl,
        const Stock::ProductQuantity& quantity, const Stock::ProductUnit& unit,
        const PurchaseMonies& monies, const RecordGroup::Flags& flags,
        const Note& note = Note());
    explicit PurchaseCartProduct(int id, const QString& product,
...
public:
    ...
    explicit PurchaseCartProductList(
        std::initializer_list<PurchaseCartProduct> productList)
