...
PurchaseCartProduct::PurchaseCartProduct(
    ...
    const Utility::Stock::ProductUnit& unit, const PurchaseMonies& monies,
...
PurchaseCartProduct::PurchaseCartProduct(int id) : id(id) {}
...
PurchaseCartProduct::PurchaseCartProduct(const QVariantMap& map)
    : id(map.value("product_id").toInt()),
      ...
      flags((map.value("tracked").toBool() ? RecordGroup::Tracked
                                           : RecordGroup::None) |
            (map.value("divisible").toBool() ? RecordGroup::Divisible
                                             : RecordGroup::None)),
      note(Note{map}),
