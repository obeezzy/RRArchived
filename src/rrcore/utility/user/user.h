...
struct User
{
    ...
    explicit User(const QString& user, const QString& password = QString());
    explicit User(int id, const QString& user = QString());
    explicit User(int id, const QString& user, const QString& firstName,
                  const QString& lastName, const QUrl& imageUrl,
                  const QString& phoneNumber, const QString& emailAddress,
                  const RecordGroup::Flags& flags, const Note& note = Note());
    explicit User(const QString& firstName, const QString& lastName,
...
public:
    ...
    UserList(std::initializer_list<User> users) : QList<User>(users) {}
    explicit UserList(const QVariantList& list) : QList<User>()
    {
...
Q_DECLARE_TYPEINFO(Utility::User::User, Q_PRIMITIVE_TYPE);
