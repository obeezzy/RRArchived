...
SoldProduct::SoldProduct(const QVariantMap& map)
    : transaction(SaleTransaction{map.value("sale_transaction_id").toInt()}),
      product(Stock::Product{map.value("product_id").toInt(),
                             map.value("product").toString()}),
      quantity(map.value("quantity").toDouble()),
      monies(SaleMonies{
          QVariantMap{{"unit_price", map.value("unit_price").toDouble()},
                      {"cost", map.value("cost").toDouble()},
                      {"discount", map.value("discount").toDouble()}}}),
      note(Note{map.value("note_id").toInt(), map.value("note").toString()}),
      timestamp(RecordTimestamp{map.value("created").toDateTime(),
                                map.value("last_edited").toDateTime()}),
      user(User::User{map.value("user_id").toInt(),
                      map.value("user").toString()}),
      row(map.value("row", -1).toInt())
{
    product.category =
        Stock::ProductCategory{map.value("product_category_id").toInt(),
                               map.value("product_category").toString()};
    product.unit = Stock::ProductUnit{map.value("product_unit_id").toInt(),
                                      map.value("product_unit").toString()};
}

QVariantMap SoldProduct::toVariantMap() const
{
    return {{"sale_transaction_id", transaction.id},
