...
SaleCartProduct::SaleCartProduct(int id) : id(id) {}

SaleCartProduct::SaleCartProduct(
    const QString& product, const Utility::Stock::ProductCategory& category,
    const QString& description, const QUrl& imageUrl,
    const Utility::Stock::ProductQuantity& quantity,
    const Stock::ProductUnit& unit, const SaleMonies& monies,
...
SaleCartProduct::SaleCartProduct(
    int id, const QString& product,
    const Utility::Stock::ProductCategory& category,
    const Utility::Stock::ProductQuantity& quantity,
    const Utility::Stock::ProductUnit& unit, const SaleMonies& monies,
...
SaleCartProduct::SaleCartProduct(const QVariantMap& map)
    : id(map.value("product_id").toInt()),
      ...
      category(Utility::Stock::ProductCategory{
          map.value("category_id").toInt(), map.value("category").toString(),
          Note{map.value("category_note_id").toInt(),
      ...
      flags((map.value("tracked").toBool() ? Utility::RecordGroup::Tracked
                                           : Utility::RecordGroup::None) |
            (map.value("divisible").toBool() ? Utility::RecordGroup::Divisible
                                             : Utility::RecordGroup::None)),
