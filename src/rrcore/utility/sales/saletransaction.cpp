...
SaleTransaction::SaleTransaction(const QVariantMap& map)
    : id(map.value("sale_transaction_id").toInt()),
      ...
      monies(SaleMonies{
          QVariantMap{{"total_cost", map.value("total_cost").toDouble()},
      ...
      flags((map.value("suspended").toBool() ? Utility::RecordGroup::Suspended
                                             : Utility::RecordGroup::None) |
            (map.value("archived").toBool() ? Utility::RecordGroup::Archived
                                            : Utility::RecordGroup::None)),
...
SaleTransaction::SaleTransaction(int id) : id(id) {}

SaleTransaction::SaleTransaction(
    int id, const Utility::Customer& customer, const SaleMonies& monies,
    const SaleCartProductList& products, const SalePaymentList& payments,
...
SaleTransaction::SaleTransaction(int id, const Utility::Customer& customer,
