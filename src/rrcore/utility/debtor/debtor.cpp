...
Debtor::Debtor(int id) : id(id) {}

Debtor::Debtor(const Client& client, const DebtTransactionList& transactions,
               const Note& note)
    : client(client),
...
Debtor::Debtor(int id, const Client& client,
               const DebtTransactionList& transactions, const Note& note)
    : id(id),
      client(client),
      transactions(transactions),
      note(note)
