...
ProductUnit::ProductUnit(const QVariantMap &map) :
    ...
    flags(map.value("preferred").toBool() ? Utility::RecordGroup::Preferred : Utility::RecordGroup::None)
{}

ProductUnit::ProductUnit(int id,
                           const QString &unit) :
    id(id),
    unit(unit)
{}

ProductUnit::ProductUnit(const QString &unit,
