...
Currency::Currency(const QString& isoCode)
    : m_code(isoCodeMap.key(isoCode.isNull() ? DEFAULT_CURRENCY : isoCode,
                            Currency::Code::Unsupported))
{
    if (!isoCode.isNull() && m_code == Code::Unsupported)
        qCWarning(lccurrency)
            << "Invalid currency code:" << isoCode << ". Code unsupported.";
}

Currency::Currency(const Currency::Code& code) : m_code(code) {}

Currency::Currency(const Currency& other) : m_code(other.code()) {}
...
{
    return Settings::instance().locale().currencySymbol(
