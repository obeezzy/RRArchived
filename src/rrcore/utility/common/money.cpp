...
Money::Money() : m_value("0.00") {}

Money::Money(const QString& num, const Currency& currency)
...
Money::Money(int num, const Currency& currency)
    : m_value(num),
      m_currency(currency)
{}

Money::Money(qlonglong num, const Currency& currency)
    : m_value(num),
      m_currency(currency)
{}

Money::Money(qulonglong num, const Currency& currency)
    : m_value(num),
      m_currency(currency)
{}

Money::Money(float num, const Currency& currency)
    : m_value(num),
      m_currency(currency)
{
    m_value.round(2);
...
Money::Money(double num, const Currency& currency)
    : m_value(num),
      m_currency(currency)
{
    m_value.round(2);
...
Money::Money(long double num, const Currency& currency)
    : m_value(num),
      m_currency(currency)
{
    m_value.round(2);
...
QString Money::toCurrencyString() const
{
    return QStringLiteral("%1%2").arg(
        m_currency.symbol(), QLocale().toString(toString().toDouble(), 'f', 2));
}
