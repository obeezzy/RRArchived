...
SaleExecutor::SaleExecutor(const QString& command, const QVariantMap& params,
                           QObject* receiver)
    : TransactionExecutor(command, params, QueryRequest::QueryGroup::Sales,
                          receiver)
{}
...
int SaleExecutor::addClient()
{
    ...
    if (records.isEmpty())
        throw UnexpectedResultException(
            QStringLiteral("Expected a client ID, received nothing."));
...
int SaleExecutor::addCustomer(int clientId)
{
    ...
    if (records.isEmpty())
        throw UnexpectedResultException(
            QStringLiteral("Expected customer ID but received nothing."));
...
int SaleExecutor::addSaleTransactionToDatabase(int clientId)
{
    ...
    if (records.isEmpty())
        throw UnexpectedResultException(
            QStringLiteral("Expected sale transaction ID, received nothing."));
    ...
    callProcedure(
        ...
        {ProcedureArgument{ProcedureArgument::Type::In, "product_id",
         ...
         ProcedureArgument{ProcedureArgument::Type::In, "user_id",
                           UserProfile::instance().userId()},
         ProcedureArgument{
             ProcedureArgument::Type::Out, "initial_quantity_id", {}}});
}
