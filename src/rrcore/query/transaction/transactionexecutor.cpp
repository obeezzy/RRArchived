...
TransactionExecutor::TransactionExecutor(
    const QString& command, const QVariantMap& params,
    const QueryRequest::QueryGroup& queryGroup, QObject* receiver)
    : QueryExecutor(command, params, queryGroup, receiver)
{}

int TransactionExecutor::addDebtor(int clientId, const QString& note,
                                   const QString& tableName)
{
    const int noteId = TransactionExecutor::addNote(
        note, tableName, ExceptionPolicy::DisallowExceptions);
    ...
    if (records.isEmpty())
        throw UnexpectedResultException(
            QStringLiteral("Expected a debtor ID, received nothing."));

    return records.first().value("debtor_id").toInt();
}

int TransactionExecutor::addDebtTransaction(int debtorId, int transactionId,
                                            const QString& note,
                                            const QString& tableName)
{
    const int noteId = TransactionExecutor::addNote(
        note, QStringLiteral("purchase_transaction"),
    ...
    if (records.isEmpty())
        throw UnexpectedResultException(
            QStringLiteral("Expected debt transaction ID, received nothing."));
                                          ...
                                          const QString& tableName)
{
    ...
    callProcedure(
        ...
        {ProcedureArgument{ProcedureArgument::Type::In, "debt_transaction_id",
         ...
         ProcedureArgument{ProcedureArgument::Type::In, "due_date_time",
...
int TransactionExecutor::addCreditor(int clientId, const QString& note,
                                     const QString& tableName)
{
    const int noteId = TransactionExecutor::addNote(
        note, tableName, ExceptionPolicy::DisallowExceptions);
    ...
    if (records.isEmpty())
        throw UnexpectedResultException(
            QStringLiteral("Expected creditor ID, received nothing."));

    return records.first().value("creditor_id").toInt();
}

int TransactionExecutor::addCreditTransaction(int creditorId, int transactionId,
                                              const QString& note,
                                              const QString& tableName)
{
    const int noteId = TransactionExecutor::addNote(
    ...
    const auto& records = callProcedure(
        ...
        {ProcedureArgument{ProcedureArgument::Type::In, "creditor_id",
                           creditorId},
         ProcedureArgument{ProcedureArgument::Type::In, "transaction_table",
                           tableName},
         ProcedureArgument{ProcedureArgument::Type::In, "transaction_id",
    ...
    if (records.isEmpty())
        throw UnexpectedResultException(QStringLiteral(
            "Expected credit transaction ID, received nothing."));
