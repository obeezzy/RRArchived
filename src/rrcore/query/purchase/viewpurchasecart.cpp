...
ViewPurchaseCart::ViewPurchaseCart(int transactionId, QObject* receiver)
...
QueryResult ViewPurchaseCart::execute()
{
    ...
    try {
        ...
        if (!products.isEmpty()) {
            ...
            outcome.insert("client_id",
                           products.first().toMap().value("client_id"));
            outcome.insert("customer_name", products.first().toMap().value(
                                                "client_preferred_name"));
            outcome.insert(
                "customer_phone_number",
                products.first().toMap().value("client_phone_number"));
            outcome.insert("total_cost",
                           products.first().toMap().value("total_cost"));
