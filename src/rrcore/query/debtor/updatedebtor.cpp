...
#include "utility/common/currency.h"
...
UpdateDebtor::UpdateDebtor(const Utility::Debtor::Debtor& debtor,
                           QObject* receiver)
...
QueryResult UpdateDebtor::execute()
{
    ...
    try {
        QueryExecutor::enforceArguments(
            {"client_id", "debtor_id", "preferred_name", "phone_number"},
            params);

        QueryExecutor::beginTransaction(q);

        affirmDirtyTransactionsExist();
        QueryExecutor::addOrUpdateNote(noteId, note, QStringLiteral("debtor"),
                                       ExceptionPolicy::DisallowExceptions);
        addNotesForNewTransactions(newDebtTransactionNoteIds);
        updateNotesForExistingTransactions();
        updateClientDetails();
        addNewTransactions(newDebtTransactionNoteIds, newDebtTransactionIds);
...
void UpdateDebtor::affirmDirtyTransactionsExist()
{
    ...
    const QVariantList& newDebtTransactions =
        params.value("new_debt_transactions").toList();
    const QVariantList& updatedDebtTransactions =
        params.value("updated_debt_transactions").toList();
    const QVariantList& archivedDebtTransactions =
        params.value("archived_debt_transactions").toList();
    const QVariantList& archivedDebtPayments =
        params.value("archived_debt_payments").toList();

    if (updatedDebtTransactions.isEmpty() && newDebtTransactions.isEmpty() &&
        archivedDebtTransactions.isEmpty() && archivedDebtPayments.isEmpty())
        throw MissingArgumentException(
            QStringLiteral(
                "No new/updated/archived debt transactions for debtor %1.")
                .arg(params.value("preferred_name").toString()));
}

void UpdateDebtor::addNotesForNewTransactions(
    QVariantList& newDebtTransactionNoteIds)
{
    const QVariantMap& params = request().params();
    const QVariantList& newDebtTransactions =
    ...
    for (const QVariant& transactionAsVariant : newDebtTransactions) {
        const QString& note =
            transactionAsVariant.toMap().value("note").toString();
        newDebtTransactionNoteIds.append(
            QueryExecutor::addNote(note, QStringLiteral("debtor"),
                                   ExceptionPolicy::DisallowExceptions));
    }

    if (newDebtTransactions.count() != newDebtTransactionNoteIds.count())
        throw ArgumentMismatchException(QStringLiteral(
...
void UpdateDebtor::updateNotesForExistingTransactions()
{
    ...
    for (const QVariant& transactionAsVariant : updatedDebtTransactions) {
        const int noteId =
            transactionAsVariant.toMap().value("note_id").toInt();
        const QString& note =
            transactionAsVariant.toMap().value("note").toString();
        QueryExecutor::addOrUpdateNote(noteId, note, QStringLiteral("debtor"),
                                       ExceptionPolicy::DisallowExceptions);
...
void UpdateDebtor::updateClientDetails()
{
    ...
    callProcedure(
        ...
        {ProcedureArgument{ProcedureArgument::Type::In, "client_id", clientId},
         ProcedureArgument{ProcedureArgument::Type::In, "first_name",
                           params.value("first_name")},
         ProcedureArgument{ProcedureArgument::Type::In, "last_name",
                           params.value("last_name")},
         ProcedureArgument{ProcedureArgument::Type::In, "preferred_name",
                           params.value("preferred_name")},
         ProcedureArgument{ProcedureArgument::Type::In, "phone_number",
                           params.value("phone_number")},
         ProcedureArgument{ProcedureArgument::Type::In, "user_id",
                           UserProfile::instance().userId()}});
}

void UpdateDebtor::addNewTransactions(
    ...
    QVariantList& newDebtTransactionIds)
{
    ...
    for (int i = 0; i < newDebtTransactions.count(); ++i) {
        const int debtTransactionId = newDebtTransactions.at(i)
                                          .toMap()
                                          .value("debt_transaction_id")
                                          .toInt();

        if (debtorId <= 0)
            throw InvalidArgumentException(
                QStringLiteral("Invalid debtor ID."));
        if (debtTransactionId <= 0)
            throw InvalidArgumentException(
                QStringLiteral("Invalid debt transaction ID."));
        ...
        callProcedure(
            ...
            {ProcedureArgument{ProcedureArgument::Type::In, "debtor_id",
             ...
             ProcedureArgument{ProcedureArgument::Type::In, "transaction_id",
                               debtTransactionId},
             ProcedureArgument{ProcedureArgument::Type::In, "note_id",
...
void UpdateDebtor::updateExistingTransactions(
    QVariantList& updatedDebtTransactionIds)
{
    const QVariantMap& params = request().params();
    const QVariantList& updatedDebtTransactions =
    ...
    for (const QVariant& transactionAsVariant : updatedDebtTransactions) {
        const int debtTransactionId =
            transactionAsVariant.toMap().value("debt_transaction_id").toInt();
...
void UpdateDebtor::archiveDeletedTransactions(
    QVariantList& archivedDebtTransactionIds)
{
    const QVariantMap& params = request().params();
    const QVariantList& archivedDebtTransactions =
    ...
    for (const QVariant& transactionAsVariant : archivedDebtTransactions) {
        ...
        callProcedure(
            "ArchiveDebtTransactionById",
            {ProcedureArgument{
                 ProcedureArgument::Type::In, "debt_transaction_id",
...
void UpdateDebtor::addNewPayments(QVariantList& newDebtPaymentIds)
{
    ...
    const QVariantList& newDebtPayments =
        params.value("new_debt_payments").toList();

    for (const QVariant& paymentAsVariant : newDebtPayments) {
        const int debtTransactionId =
            paymentAsVariant.toMap().value("debt_transaction_id").toInt();
        const double totalAmount =
            paymentAsVariant.toMap().value("total_amount").toDouble();
        const double amountPaid =
            paymentAsVariant.toMap().value("amount_paid").toDouble();
        const double balance =
            paymentAsVariant.toMap().value("balance").toDouble();
        const QDateTime& dueDateTime =
            paymentAsVariant.toMap().value("due_date").toDateTime();
        ...
        if (debtTransactionId <= 0)
            throw InvalidArgumentException(
                QStringLiteral("Debt transaction ID for payment is invalid."));
        if (totalAmount < amountPaid)
            throw InvalidArgumentException(
                QStringLiteral("Amount paid cannot exceed total debt."));
        if (balance != (totalAmount - amountPaid))
            throw InvalidArgumentException(QStringLiteral("Invalid balance."));
        if (dueDateTime.isNull() || !dueDateTime.isValid() ||
            dueDateTime <= QDateTime::currentDateTime())
        ...
        const QList<QSqlRecord>& records = callProcedure(
            ...
            {ProcedureArgument{ProcedureArgument::Type::In,
                               "debt_transaction_id", debtTransactionId},
             ProcedureArgument{ProcedureArgument::Type::In, "total_amount",
                               totalAmount},
             ProcedureArgument{ProcedureArgument::Type::In, "amount_paid",
                               amountPaid},
             ProcedureArgument{ProcedureArgument::Type::In, "balance",
                               qAbs(balance)},
             ProcedureArgument{ProcedureArgument::Type::In, "currency",
                               currency},
             ProcedureArgument{ProcedureArgument::Type::In, "due_date",
                               dueDateTime},
        ...
        if (records.isEmpty())
            throw UnexpectedResultException(QStringLiteral(
                "Expected a debt payment ID but got nothing back."));

        const int debtPaymentId =
            records.first().value("debt_payment_id").toInt();
...
{
    const QVariantMap& params = request().params();
    const QVariantList& updatedDebtPayments =
        params.value("updated_debt_payments").toList();

    for (const QVariant& paymentAsVariant : updatedDebtPayments) {
        const int debtPaymentId =
            paymentAsVariant.toMap().value("debt_payment_id").toInt();
        const double totalAmount =
            paymentAsVariant.toMap().value("total_amount").toDouble();
        const double amountPaid =
            paymentAsVariant.toMap().value("amount_paid").toDouble();
        ...
        if (debtPaymentId <= 0)
            throw InvalidArgumentException(
                QStringLiteral("Invalid debt payment ID."));
        if (dueDateTime.isNull() || !dueDateTime.isValid() ||
            dueDateTime <= QDateTime::currentDateTime())
            throw InvalidDueDateException();
        if (balance < 0.0)
