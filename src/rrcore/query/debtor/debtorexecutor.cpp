...
void DebtorExecutor::arrangeDebtTransactions(
    const Utility::Debtor::DebtTransactionList& debtTransactions)
{
    ...
    for (const auto& transaction : debtTransactions) {
        if (transaction.state == Utility::Debtor::DebtTransaction::State::Fresh)
            ...
            updatedDebtTransactions.append(transaction.toVariantMap());
                 ...
                 Utility::Debtor::DebtTransaction::State::Trash)
    ...
    if (!updatedDebtTransactions.isEmpty())
        request().params().insert("updated_debt_transactions",
                                  updatedDebtTransactions);
    if (!archivedDebtTransactions.isEmpty())
        request().params().insert("archived_debt_transactions",
                                  archivedDebtTransactions);
    ...
    if (!archivedDebtPayments.isEmpty())
        request().params().insert("archived_debt_payments",
                                  archivedDebtPayments);
}
