...
AddDebtor::AddDebtor(const Utility::Debtor::Debtor& debtor, QObject* receiver)
...
QueryResult AddDebtor::addDebtor()
{
    ...
    const QString& phoneNumber =
        request().params().value("phone_number").toString();
    ...
    try {
        QueryExecutor::enforceArguments({"preferred_name", "phone_number"},
                                        params);

        if (newDebtTransactions.count() == 0)
            throw MissingArgumentException(
                QStringLiteral("No new debt transactions for debtor %1.")
                    .arg(params.value("preferred_name").toString()));

        QueryExecutor::beginTransaction(q);

        affirmDebtorDoesNotExist(phoneNumber);
        noteId = addNotesForDebtor(params, newDebtTransactions,
                                   debtTransactionNoteIds);
        clientId = addClient(params, noteId);
        addDebtorToDatabase(clientId, noteId);
        addDebtTransactions(debtorId, newDebtTransactions,
                            debtTransactionNoteIds, debtTransactionIds,
                            debtPaymentIds);
...
int AddDebtor::addClient(const QVariantMap& params, int noteId)
{
    ...
    const auto& records(callProcedure(
        ...
        {ProcedureArgument{ProcedureArgument::Type::In, "first_name",
         ...
         ProcedureArgument{ProcedureArgument::Type::In, "address",
...
int AddDebtor::addDebtorToDatabase(int clientId, int noteId)
{
    const auto& records = callProcedure(
        "AddDebtor",
                                    ...
                                    QVariantList& newDebtPaymentIds)
{
    for (int i = 0; i < newDebtTransactions.count(); ++i) {
        ...
        const int debtTransactionId =
            records.first().value("debt_transaction_id").toInt();
        newDebtTransactionIds.append(debtTransactionId);
        addDebtPayments(debtTransactionId,
                        newDebtTransactions.at(i)
                            .toMap()
                            .value("new_debt_payments")
                            .toList(),
                        newDebtPaymentIds);
    }
}
                                ...
                                QVariantList& newDebtPaymentIds)
{
    for (const QVariant& debtPaymentAsVariant : newDebtPayments) {
        const QDateTime& dueDateTime =
            debtPaymentAsVariant.toMap().value("due_date_time").toDateTime();
        const double totalDebt =
            debtPaymentAsVariant.toMap().value("total_debt").toDouble();
        const double amountPaid =
            debtPaymentAsVariant.toMap().value("amount").toDouble();
        const double balance =
            debtPaymentAsVariant.toMap().value("balance").toDouble();
        const QString& currency = DEFAULT_CURRENCY;
        const QString& note =
            debtPaymentAsVariant.toMap().value("note").toString();
        const int noteId =
            QueryExecutor::addNote(note, QStringLiteral("debtor"),
        ...
        const auto& records = callProcedure(
            ...
            {ProcedureArgument{ProcedureArgument::Type::In,
             ...
             ProcedureArgument{ProcedureArgument::Type::In, "total_debt",
                               totalDebt},
