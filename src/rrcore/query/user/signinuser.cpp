...
SignInUser::SignInUser(const Utility::User::User& user, QObject* receiver)
...
QueryResult SignInUser::execute()
{
    ...
    QueryExecutor::enforceArguments({"user", "password"}, request().params());
...
bool SignInUser::storeProfile(QueryResult& result, const QString& userName,
                              const QString& password)
{
    ...
    try {
        const auto& records(callProcedure(
            "FetchUserByName", {ProcedureArgument{ProcedureArgument::Type::In,
                                                  "user", userName}}));

        if (records.isEmpty())
            throw NoExistingRecordException(
                QStringLiteral("User '%1' does not exist in RR user table.")
                    .arg(userName));

        const QVariantMap& user{recordToMap(records.first())};
        result.setOutcome(
            QVariantMap{{"user", user},
                        {"password", password},
...
void SignInUser::attemptSignIn(QueryResult& result)
{
    ...
    if (!connection.open() || !storeProfile(result, userName, password)) {
        if (connection.lastError().nativeErrorCode().toInt() ==
            static_cast<int>(
                DatabaseError::MySqlErrorCode::UserAccountIsLockedError))
            throw UserAccountLockedException(
                QStringLiteral("User account for '%1' is locked.")
                    .arg(userName),
                connection.lastError());
        else
            throw InvalidCredentialsException(
                QStringLiteral("Failed to sign in as '%1'.").arg(userName),
                connection.lastError());
    }
}
