...
UserExecutor::UserExecutor(const QString& command, const QVariantMap& params,
                           QObject* receiver)
...
void UserExecutor::createRRUser(const QString& userName, QSqlQuery& q)
{
    ...
    q.prepare(
        QStringLiteral("INSERT INTO %1.user (user, photo, phone_number, "
                       "email_address, active, pending, created, last_edited) "
                       "VALUES (:user, :photo, :phone_number, :email_address, "
                       ":active, :pending, :created, :last_edited)")
    ...
    if (!q.exec())
        throw FailedToCreateUserException(
            QStringLiteral("Failed to create RR user '%1'.").arg(userName),
            q.lastError());
}

void UserExecutor::grantPrivilege(const QString& privilege,
                                  const QString& userName, QSqlQuery& q)
{
    q.prepare(
        QString("GRANT %1 ON * . * TO :username@'localhost'").arg(privilege));
    q.bindValue(":username", userName);
    if (!q.exec())
        throw FailedToCreateUserException(
            QStringLiteral("Failed to grant '%1' privileges to '%2'.")
