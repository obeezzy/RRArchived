...
QueryResult UpdateProduct::execute()
{
    ...
    if (params.contains("quantity"))
        qCWarning(lcupdateproduct)
            << Q_FUNC_INFO
            << "-> This function is not responsible for updating quantity. "
               "Quantity will be ignored.";
...
int UpdateProduct::addOrUpdateProductCategory()
{
    ...
    if (records.isEmpty())
        throw UnexpectedResultException(
            QStringLiteral("Expected category ID, received nothing."));
...
void UpdateProduct::updateProductUnit()
{
    ...
    const int noteId =
        QueryExecutor::addNote(note, QStringLiteral("product_unit"),
                               ExceptionPolicy::DisallowExceptions);
