...
FetchProductCount::FetchProductCount(int categoryId, QObject* receiver)
    : StockExecutor(COMMAND, {{"category_id", categoryId}, {"archived", false}},
                    receiver)
{}
...
QueryResult FetchProductCount::execute()
{
    ...
    try {
        const QList<QSqlRecord> records(callProcedure(
            "FetchProductCount",
            {ProcedureArgument{ProcedureArgument::Type::In, "category_id",
