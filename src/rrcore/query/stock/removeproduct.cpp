...
RemoveProduct::RemoveProduct(const QueryRequest& request, QObject* receiver)
    ...
    try {
        ...
        if (records.isEmpty())
            throw UnexpectedResultException(
                "Expected archived product, received nothing.");

        QVariantMap outcome;
        result.setOutcome(QVariantMap{
            {"product_category_id",
             records.first().value("product_category_id").toInt()},
