...
FetchProduct::FetchProduct(int productId, QObject* receiver)
    : StockExecutor(COMMAND, {{"product_id", productId}}, receiver)
{}
    ...
    try {
        ...
        if (records.isEmpty())
            throw NoExistingRecordException(
                QStringLiteral(
                    "Product details do not exists for product '%1'.")
                    .arg(params.value("product_id").toString()));

        QVariantMap product{recordToMap(records.first())};
        product.insert("image_url", QueryExecutor::byteArrayToImageUrl(
                                        product.value("image").toByteArray()));
        product.remove("image");
        result.setOutcome(
            QVariantMap{{"product", product}, {"record_count", 1}});
        return result;
