...
BusinessStoreModel::BusinessStoreModel(QObject* parent)
    : QAbstractListModel(parent),
      m_currentIndex(-1)
{
    connect(UserProfile::instance().businessAdmin(),
            &BusinessAdmin::businessStoresChanged, this,
            &BusinessStoreModel::reset);
}
...
{
    return {{BusinessStoreId, "business_store_id"},
            {NameRole, "name"},
            {AddressRole, "address"},
...
BusinessStore BusinessStoreModel::currentBusinessStore() const
{
    if (m_currentIndex < -1 ||
        m_currentIndex >=
            UserProfile::instance().businessAdmin()->businessStores().count())
        return BusinessStore();

    return UserProfile::instance().businessAdmin()->businessStores().at(
        m_currentIndex);
}
