...
void QMLDebtorModel::processResult(const QueryResult &result)
{
    ...
    if (result.isSuccessful()) {
        if (result.request().command() == Query::Debtor::ViewDebtors::COMMAND) {
            beginResetModel();
            m_debtorList = Utility::Debtor::DebtorList{ result.outcome().toMap().value("debtors").toList() };
            endResetModel();
            emit success(ModelResult{ ViewDebtorsSuccess });
        } else if (result.request().command() == Query::Debtor::RemoveDebtor::COMMAND) {
        ...
        } else if (result.request().command() == Query::Debtor::RemoveDebtor::UNDO_COMMAND) {
            const auto &debtor = Utility::Debtor::Debtor{ result.outcome().toMap() };
...
void QMLDebtorModel::filter()
{
    ...
    setBusy(true);
    emit execute(new Query::Debtor::FilterDebtors(filterCriteria(),
                                                this));
}
...
void QMLDebtorModel::removeDebtorFromModel(const Utility::Debtor::Debtor &debtor)
{
...
void QMLDebtorModel::undoRemoveDebtorFromModel(const Utility::Debtor::Debtor &debtor)
{
...
void QMLDebtorModel::undoLastCommit()
{
    ...
    if (lastSuccessfulRequest().command() == Query::Debtor::RemoveDebtor::COMMAND) {
        setBusy(true);
