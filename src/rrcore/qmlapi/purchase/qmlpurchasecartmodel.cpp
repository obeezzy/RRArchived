...
{
    connect(this, &QMLPurchaseCartModel::transactionIdChanged, this,
            &QMLPurchaseCartModel::tryQuery);
}
...
void QMLPurchaseCartModel::setCustomerPhoneNumber(
...
QString QMLPurchaseCartModel::toPrintableFormat() const
{
    ...
    rootObject.insert("transactions",
...
void QMLPurchaseCartModel::addTransaction(const QVariantMap& transaction)
{
    ...
    if (!m_transaction.products.isEmpty()) {
        setBusy(true);
        emit execute(
            new Query::Purchase::AddPurchaseTransaction(m_transaction, this));
    } else {
        emit error(ModelResult{EmptyCartError});
    }
}

void QMLPurchaseCartModel::updateSuspendedTransaction(
    const QVariantMap& transactionAddOns)
{
    ...
    if (!m_transaction.products.isEmpty())
        emit execute(new Query::Purchase::UpdateSuspendedPurchaseTransaction(
            m_transaction, this));
    else
        emit error(ModelResult{EmptyCartError});
}
...
void QMLPurchaseCartModel::tryQuery()
{
    if (m_transaction.id > 0) {
        setBusy(true);
        emit execute(
            new Query::Purchase::ViewPurchaseCart(m_transaction.id, this));
    } else {
        beginResetModel();
        setCustomerName(QString());
        setCustomerPhoneNumber(QString());
...
void QMLPurchaseCartModel::processResult(const QueryResult& result)
{
    ...
    disconnect(this, &QMLPurchaseCartModel::transactionIdChanged, this,
               &QMLPurchaseCartModel::tryQuery);

    if (result.isSuccessful()) {
        beginResetModel();

        m_transaction.products = Utility::Purchase::PurchaseCartProductList{
            result.outcome().toMap().value("products").toList()};
        calculateTotalCost();
        endResetModel();

        if (result.request().command() ==
            Query::Purchase::AddPurchaseTransaction::COMMAND) {
            if (result.request().params().value("suspended").toBool()) {
                ...
                emit success(ModelResult{SubmitTransactionSuccess});
                   ...
                   Query::Purchase::ViewPurchaseCart::COMMAND) {
            const auto vendor = Utility::Vendor{result.outcome().toMap()};
            ...
            emit success(ModelResult{RetrieveTransactionSuccess});
                   ...
                   Query::Purchase::UpdateSuspendedPurchaseTransaction::
                       COMMAND) {
            const auto vendor = Utility::Vendor{result.outcome().toMap()};
            setTransactionId(0);
            setVendorId(vendor.id);
            ...
            emit success(ModelResult{SuspendTransactionSuccess});
                   ...
                   Query::Purchase::AddPurchaseTransaction::UNDO_COMMAND) {
        ...
        if (result.request().command() ==
            Query::Purchase::AddPurchaseTransaction::COMMAND) {
            const auto transaction = Utility::Purchase::PurchaseTransaction{
...
void QMLPurchaseCartModel::addProduct(const QVariantMap& product)
{
    ...
    const auto& availableQuantity = Utility::Stock::ProductQuantity{
        qMin(1.0, product.value("available_quantity").toDouble())};

    if (availableQuantity.isZero())
        return;

    if (!m_transaction.products.contains(
            Utility::Purchase::PurchaseCartProduct{productId})) {
        beginInsertRows(QModelIndex(), rowCount(), rowCount());
        const auto newProduct = Utility::Purchase::PurchaseCartProduct{product};
        m_transaction.products.append(newProduct);
        endInsertRows();
    } else {
        const auto row = m_transaction.products.indexOf(
            Utility::Purchase::PurchaseCartProduct{productId});
        auto& existingProduct = m_transaction.products[row];
        const auto& oldQuantity = existingProduct.quantity;
        const auto& newQuantity =
            oldQuantity + Utility::Stock::ProductQuantity(1.0);
...
void QMLPurchaseCartModel::setProductQuantity(int productId, double quantity)
{
    ...
    if (oldQuantity != Utility::Stock::ProductQuantity(quantity)) {
        ...
        existingProduct.monies.cost =
            Utility::Money{quantity * unitPrice.toDouble()};
