...
{
    setBusy(true);
    emit execute(new Query::Purchase::ViewPurchaseReport(dateTimeSpan(),
                                                           this));
}
...
void QMLPurchaseReportModel::processResult(const QueryResult &result)
{
    ...
    if (result.isSuccessful()) {
        if (result.request().command() == Query::Purchase::ViewPurchaseReport::COMMAND) {
            beginResetModel();
            m_transactions = Utility::Purchase::PurchaseReportTransactionList{ result.outcome().toMap().value("transactions").toList() };
            endResetModel();
            emit success();
        } else {
            emit error();
