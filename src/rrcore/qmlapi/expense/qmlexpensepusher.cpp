...
#include "query/expense/addexpensetransaction.h"
...
QMLExpensePusher::PaymentMethod QMLExpensePusher::paymentMethod() const
{
    switch (m_transaction.paymentMethod.enumValue()) {
        ...
        default:
            qCWarning(lcqmlexpensepusher)
                << "Invalid Utility::PaymentMethod enum value. "
...
void QMLExpensePusher::push()
{
    setBusy(true);
    emit execute(
        new Query::Expense::AddExpenseTransaction(m_transaction, this));
}
...
void QMLExpensePusher::processResult(const QueryResult result)
{
    ...
    if (result.isSuccessful()) {
        if (result.request().command() ==
            Query::Expense::AddExpenseTransaction::COMMAND)
