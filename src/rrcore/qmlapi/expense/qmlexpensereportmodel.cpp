...
{
    setBusy(true);
    emit execute(new Query::Expense::ViewExpenseReport(dateTimeSpan(),
                                                         this));
}
...
void QMLExpenseReportModel::processResult(const QueryResult &result)
{
    ...
    if (result.isSuccessful()) {
        if (result.request().command() == Query::Expense::ViewExpenseReport::COMMAND) {
            beginResetModel();
            m_transactions = Utility::Expense::ExpenseReportTransactionList{ result.outcome().toMap().value("transactions").toList() };
            endResetModel();
            emit success();
        } else {
            emit error();
