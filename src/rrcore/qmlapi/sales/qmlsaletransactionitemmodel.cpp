...
void QMLSaleTransactionItemModel::tryQuery()
{
    ...
    setBusy(true);
    emit execute(new Query::Sales::ViewSoldProducts(transactionId(),
                                                 this));
}
...
void QMLSaleTransactionItemModel::processResult(const QueryResult &result)
{
    ...
    if (result.isSuccessful()) {
        if (result.request().command() == Query::Sales::ViewSoldProducts::COMMAND) {
            beginResetModel();
            m_products = Utility::Sales::SoldProductList{ result.outcome().toMap().value("products").toList() };
            endResetModel();
