...
QVariant QMLSaleReportModel::headerData(int section,
...
{
    setBusy(true);
    emit execute(new Query::Sales::ViewSaleReport(dateTimeSpan(), this));
}
...
void QMLSaleReportModel::processResult(const QueryResult& result)
{
    ...
    if (result.isSuccessful()) {
        if (result.request().command() ==
            Query::Sales::ViewSaleReport::COMMAND) {
            beginResetModel();
            m_transactions = Utility::Sales::SaleReportTransactionList{
