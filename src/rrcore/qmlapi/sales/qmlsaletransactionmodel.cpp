...
{
    return {{SaleTransactionIdRole, "sale_transaction_id"},
...
QVariant QMLSaleTransactionModel::headerData(int section,
...
void QMLSaleTransactionModel::tryQuery()
{
    ...
    emit execute(
        new Query::Sales::ViewSaleTransactions(dateTimeSpan(), flags, this));
}
...
void QMLSaleTransactionModel::processResult(const QueryResult& result)
{
    ...
    if (result.isSuccessful()) {
        if (result.request().command() ==
            Query::Sales::ViewSaleTransactions::COMMAND) {
            beginResetModel();
            m_transactions = Utility::Sales::SaleTransactionList{
