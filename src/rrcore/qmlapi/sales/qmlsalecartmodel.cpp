...
{
    connect(this, &QMLSaleCartModel::transactionIdChanged, this,
            &QMLSaleCartModel::tryQuery);
    connect(this, &QMLSaleCartModel::totalCostChanged, this,
            &QMLSaleCartModel::calculateBalance);
    connect(this, &QMLSaleCartModel::amountPaidChanged, this,
            &QMLSaleCartModel::calculateBalance);
}
...
QVariant QMLSaleCartModel::data(const QModelIndex& index, int role) const
{
    ...
    switch (role) {
        ...
        case AvailableQuantityRole:
            return m_transaction.products.at(index.row())
                .availableQuantity.toDouble();
        case QuantityRole:
        ...
        case CostPriceRole:
            return m_transaction.products.at(index.row())
                .monies.costPrice.toDouble();
        case RetailPriceRole:
            return m_transaction.products.at(index.row())
                .monies.retailPrice.toDouble();
        case UnitPriceRole:
            return m_transaction.products.at(index.row())
                .monies.unitPrice.toDouble();
        case CostRole:
            return m_transaction.products.at(index.row())
                .monies.cost.toDouble();
    }
...
void QMLSaleCartModel::setCustomerPhoneNumber(
...
{
    if (!m_transaction.monies.balance.isZero() &&
        transactionInfo.value("due_date_time").isNull())
...
QString QMLSaleCartModel::toPrintableFormat() const
{
    ...
    rootObject.insert("phone_number",
                      m_transaction.customer.client.phoneNumber);
    rootObject.insert("query_group", "sales");
    rootObject.insert("records", QJsonArray::fromVariantList(
                                     m_transaction.products.toVariantList()));
...
void QMLSaleCartModel::addTransaction(const QVariantMap& transaction)
{
    if (!m_transaction.products.isEmpty()) {
        setBusy(true);
        m_transaction.customer.client.id =
...
void QMLSaleCartModel::processResult(const QueryResult& result)
{
    ...
    disconnect(this, &QMLSaleCartModel::transactionIdChanged, this,
               &QMLSaleCartModel::tryQuery);

    if (result.isSuccessful()) {
        beginResetModel();
        m_transaction.products = Utility::Sales::SaleCartProductList{
            result.outcome().toMap().value("products").toList()};
        endResetModel();

        if (result.request().command() ==
            Query::Sales::AddSaleTransaction::COMMAND) {
            if (result.request().params().value("suspended").toBool()) {
                setTransactionId(0);
                setCustomerName(QString());
                setCustomerPhoneNumber(QString());
                setCustomerId(
                    result.outcome().toMap().value("customer_id").toInt());
                ...
                emit success(ModelResult{SubmitTransactionSuccess});
                   ...
                   Query::Sales::AddSaleTransaction::UNDO_COMMAND) {
            setTransactionId(0);
            setCustomerName(QString());
            setCustomerPhoneNumber(QString());
            setCustomerId(
            ...
            emit success(ModelResult{UndoSubmitTransactionSuccess});
                   ...
                   Query::Sales::ViewSaleCart::COMMAND) {
            setCustomerId(
                result.outcome().toMap().value("customer_id").toInt());
            setCustomerName(
                result.outcome().toMap().value("customer_name").toString());
            setCustomerPhoneNumber(result.outcome()
                                       .toMap()
                                       .value("customer_phone_number")
                                       .toString());
            emit success(ModelResult{RetrieveTransactionSuccess});
        } else if (result.request().command() ==
                   Query::Sales::UpdateSuspendedSaleTransaction::COMMAND) {
            setTransactionId(0);
            setCustomerId(
                result.outcome().toMap().value("customer_id").toInt());
            setCustomerName(
...
void QMLSaleCartModel::addProduct(const QVariantMap& product)
{
    ...
    if (newProduct.availableQuantity.isZero()) {
        qCWarning(lcqmlsalecartmodel)
            << Q_FUNC_INFO << "Available quantity is zero. Aborting...";
        return;
    }

    if (!m_transaction.products.contains(newProduct)) {
        beginInsertRows(QModelIndex(), m_transaction.products.count(),
                        m_transaction.products.count());
        m_transaction.products.append(newProduct);
        calculateTotalCost();
        endInsertRows();
    } else {
        const int row = m_transaction.products.indexOf(newProduct);
        Utility::Sales::SaleCartProduct& existingProduct =
            m_transaction.products[row];
        const Utility::Stock::ProductQuantity& oldQuantity =
            existingProduct.quantity;
        const Utility::Stock::ProductQuantity& newQuantity =
            oldQuantity + newProduct.quantity;

        existingProduct.quantity =
            qMin(newQuantity, existingProduct.availableQuantity);
        existingProduct.monies.cost =
            Utility::Money(existingProduct.quantity.toDouble() *
                           existingProduct.monies.unitPrice.toDouble());
...
void QMLSaleCartModel::updateProduct(int productId, const QVariantMap& product)
{
    ...
    const Utility::Stock::ProductQuantity& oldQuantity =
        existingProduct.quantity;
    const Utility::Stock::ProductQuantity& availableQuantity =
...
void QMLSaleCartModel::setProductQuantity(int productId, double quantity)
{
    ...
    const int row = m_transaction.products.indexOf(
        Utility::Sales::SaleCartProduct{productId});
    Utility::Sales::SaleCartProduct& existingProduct{
        m_transaction.products[row]};
    const Utility::Stock::ProductQuantity& oldQuantity =
        existingProduct.quantity;
    const Utility::Stock::ProductQuantity& availableQuantity =
        existingProduct.availableQuantity;
    const Utility::Stock::ProductQuantity& newQuantity =
        qMin(Utility::Stock::ProductQuantity(quantity), availableQuantity);
    const Utility::Money& unitPrice = existingProduct.monies.unitPrice;

    if (oldQuantity != newQuantity) {
        existingProduct.quantity = newQuantity;
        existingProduct.monies.cost =
            Utility::Money{newQuantity.toDouble() * unitPrice.toDouble()};
        emit dataChanged(index(row), index(row));
    }
}
...
{
    setBalance(m_transaction.monies.totalCost.toDouble() -
               m_transaction.monies.amountPaid.toDouble());
}
