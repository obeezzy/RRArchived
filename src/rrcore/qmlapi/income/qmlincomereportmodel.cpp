...
QVariant QMLIncomeReportModel::headerData(int section,
...
{
    setBusy(true);
    emit execute(new Query::Income::ViewIncomeReport(dateTimeSpan(), this));
}
...
void QMLIncomeReportModel::processResult(const QueryResult& result)
{
    ...
    if (result.isSuccessful()) {
        if (result.request().command() ==
            Query::Income::ViewIncomeReport::COMMAND) {
            beginResetModel();
            m_transactions = Utility::Income::IncomeReportTransactionList{
