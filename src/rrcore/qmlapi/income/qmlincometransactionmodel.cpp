...
{
    setBusy(true);
    emit execute(new Query::Income::ViewIncomeTransactions(dateTimeSpan(),
...
void QMLIncomeTransactionModel::processResult(const QueryResult &result)
{
    ...
    if (result.isSuccessful()) {
        m_transactions = Utility::Income::IncomeTransactionList{ result.outcome().toMap().value("transactions").toList() };
        emit success();
    } else {
        emit error();
    }
