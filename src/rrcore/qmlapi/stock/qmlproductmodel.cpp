...
{
    connect(this, &QMLProductModel::categoryIdChanged, this,
            &QMLProductModel::tryQuery);
}
    ...
    switch (role) {
        ...
        case DivisibleRole:
            return m_products.at(index.row())
                .flags.testFlag(Utility::RecordGroup::Divisible);
        case ImageUrlRole:
...
{
    return {{CategoryIdRole, "product_category_id"},
...
QVariant QMLProductModel::headerData(int section, Qt::Orientation orientation,
                                     int role) const
{
...
void QMLProductModel::removeProduct(int row)
{
    ...
    product.row = row;
    emit execute(new Query::Stock::RemoveProduct(product, this));
}
...
void QMLProductModel::tryQuery()
{
    ...
    if (filterCriteria().isValid() && sortCriteria().isValid()) {
        emit execute(new Query::Stock::FilterProducts(
            filterCriteria(), sortCriteria(), m_categoryId, this));

    } else {
        emit execute(
            new Query::Stock::ViewProducts(m_categoryId, sortCriteria(), this));
    }
}
    ...
    if (result.isSuccessful()) {
        if (result.request().command() == Query::Stock::ViewProducts::COMMAND ||
            ...
            const auto product = Utility::Stock::Product{
                result.request().params().value("product").toMap()};
...
void QMLProductModel::undoRemoveProductFromModel(
    const Utility::Stock::Product& product)
{
