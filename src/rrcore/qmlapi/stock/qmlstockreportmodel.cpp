...
#include "query/stock/viewstockreport.h"
...
{
    setBusy(true);
    emit execute(new Query::Stock::ViewStockReport(dateTimeSpan(),
                                                     this));
}
...
void QMLStockReportModel::processResult(const QueryResult &result)
{
    ...
    if (result.isSuccessful()) {
        if (result.request().command() == Query::Stock::ViewStockReport::COMMAND) {
            beginResetModel();
            m_transactions = Utility::Stock::StockReportTransactionList{ result.outcome().toMap().value("transactions").toList() };
            endResetModel();
            emit success();
        }
    } else {
