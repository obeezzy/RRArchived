...
{
    Q_OBJECT
    Q_PROPERTY(QString productFilterText READ productFilterText WRITE
                   setProductFilterText NOTIFY productFilterTextChanged)
public:
    explicit QMLProductCategoryModel(QObject* parent = nullptr);
    explicit QMLProductCategoryModel(DatabaseThread& thread,
...
private:
    ...
    void undoRemoveCategoryFromModel(int row, const QVariantMap& category);
