...
#include "query/user/changepassword.h"
#include "query/user/signinuser.h"
#include "query/user/signoutuser.h"
#include "query/user/signupuser.h"
      ...
      m_businessStoreModel(new BusinessStoreModel(this))
{
    ...
    connect(&thread, &DatabaseThread::resultReady, this,
            &QMLUserProfile::processResult);

    connect(this, &QMLUserProfile::executeRequest, &NetworkThread::instance(),
            QOverload<ServerRequest>::of(&NetworkThread::execute));
    connect(&NetworkThread::instance(), &NetworkThread::responseReady, this,
            &QMLUserProfile::processServerResponse);
...
void QMLUserProfile::signIn(const QString& userName, const QString& password)
{
    ...
    if (userName.trimmed().isEmpty()) {
        ...
        setBusy(true);
        emit execute(new Query::User::SignInUser(
            Utility::User::User{userName, password}, this));
    }
}
...
void QMLUserProfile::signUp(const QString& userName, const QString& password)
{
    ...
    if (userName.trimmed().isEmpty()) {
        ...
        setBusy(true);
        emit execute(new Query::User::SignUpUser(
            Utility::User::User{userName, password}, this));
    }
}
...
void QMLUserProfile::signOut()
{
    ...
    setBusy(true);
    emit execute(new Query::User::SignOutUser(this));
}
                                 ...
                                 const QString& password)
{
    ...
    if (emailAddress.trimmed().isEmpty()) {
        ...
        request.setAction("link_account", {{"email_address", emailAddress},
                                           {"password", password}});
...
void QMLUserProfile::linkBusinessStore()
{
    ...
    const BusinessStore& businessStore(
        UserProfile::instance().businessAdmin()->businessStores().at(
            m_businessStoreModel->currentIndex()));
    ServerRequest request(this);
    request.setAction(
        "link_business_store",
        {{"business_store", businessStore.toVariantMap()},
         {"email_address",
          UserProfile::instance().businessAdmin()->emailAddress()},
         {"password", UserProfile::instance().businessAdmin()->password()}});
    emit executeRequest(request);
}

void QMLUserProfile::changePassword(const QString& oldPassword,
                                    const QString& newPassword)
{
    ...
    emit execute(
        new Query::User::ChangePassword(oldPassword, newPassword, this));
}
...
void QMLUserProfile::processResult(const QueryResult& result)
{
    ...
    if (result.isSuccessful()) {
        if (result.request().command() == Query::User::SignInUser::COMMAND) {
        ...
        switch (result.errorCode()) {
            case DatabaseError::asInteger(
                DatabaseError::QueryErrorCode::UserAccountIsLocked):
...
void QMLUserProfile::processServerResponse(const ServerResponse& response)
{
    ...
    if (response.isSuccessful()) {
        if (response.request().action() == QStringLiteral("link_account")) {
            UserProfile::instance().businessAdmin()->extractFromVariantMap(
                response.data().value("business_admin").toMap());
            emit success(ModelResult{LinkAccountSuccess});
        } else if (response.request().action() ==
                   QStringLiteral("link_business_store")) {
            const BusinessStore& businessStore(BusinessStore::fromVariantMap(
                response.data().value("business_store").toMap()));

            UserProfile::instance().setUserId(1);  // User is definitely "admin"
            UserProfile::instance().setRackId(businessStore.rackId());
            UserProfile::instance().businessDetails()->extractFromBusinessStore(
                businessStore);
            UserProfile::instance().businessAdmin()->setPassword(
                QString());  // Clear password from memory
            emit success(ModelResult{LinkBusinessStoreSuccess});
        }
    } else {
        if (!response.hasError()) {
            switch (response.statusCode()) {
                case NetworkError::asInteger(
                    NetworkError::StatusCode::ConnectionRefusedError):
            ...
            switch (response.errorCode()) {
                case NetworkError::asInteger(
                    NetworkError::ServerErrorCode::IncorrectCredentialsError):
                    emit error(ModelResult{IncorrectCredentials});
                    break;
                default:
                    emit error();
...
{
    const BusinessStore& businessStore(
        m_businessStoreModel->currentBusinessStore());
    UserProfile::instance().businessDetails()->extractFromBusinessStore(
        businessStore);
}
