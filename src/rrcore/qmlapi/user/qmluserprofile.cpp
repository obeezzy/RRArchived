                            ...
                            const QString &password)
{
    ...
    if (userName.trimmed().isEmpty()) {
        ...
        setBusy(true);
        emit execute(new Query::User::SignInUser(Utility::User::User {
                                                   userName,
                                                   password
                                               }, this));
    }
                            ...
                            const QString &password)
{
    ...
    if (userName.trimmed().isEmpty()) {
        ...
        setBusy(true);
        emit execute(new Query::User::SignUpUser(Utility::User::User {
                                                   userName,
                                                   password
                                               }, this));
    }
...
void QMLUserProfile::changePassword(const QString &oldPassword, const QString &newPassword)
{
    ...
    emit execute(new Query::User::ChangePassword(oldPassword, newPassword, this));
}
...
void QMLUserProfile::processResult(const QueryResult &result)
{
    ...
    if (result.isSuccessful()) {
        ...
        } else if (result.request().command() == Query::User::SignUpUser::COMMAND) {
            const auto user = Utility::User::User{ result.outcome().toMap() };
            UserProfile::instance().setUser(user);
            emit success(ModelResult{ SignUpSuccess });
        } else if (result.request().command() == Query::User::SignOutUser::COMMAND) {
            UserProfile::instance().clearUser();
            emit success(ModelResult{ SignOutSuccess });
        } else if (result.request().command() == Query::User::ChangePassword::COMMAND) {
            UserProfile::instance().setDatabaseReady(true);
