...
ProcedureCallFailedException::ProcedureCallFailedException(
    const QString& message, const QSqlError& error)
    : DatabaseException(DatabaseError::QueryErrorCode::ProcedureFailed, message,
                        error)
{}
...
QString ProcedureCallFailedException::toString() const
{
    ...
    return QStringLiteral("ProcedureCallFailedException(%1)").arg(message());
}
