...
{
    if (m_command.startsWith("sign") || m_command == "change_password")
        return CommandVerb::Authenticate;
    else if (m_command.startsWith("view") || m_command.startsWith("fetch") ||
             m_command.startsWith("filter"))
        return CommandVerb::Read;
    else if (m_command.startsWith("update") || m_command.startsWith("change") ||
             m_command.startsWith("deduct"))
        return CommandVerb::Update;
    else if (m_command.startsWith("archive") || m_command.startsWith("undo"))
...
{
    QJsonObject jsonObject{{"command", m_command},
...
QueryRequest::QueryGroup QueryRequest::queryGroupToEnum(
    const QString& queryGroupString)
{
...
QString QueryRequest::queryGroupToString(
    QueryRequest::QueryGroup queryGroupEnum)
{
...
void QueryRequest::setCommand(const QString& command, const QVariantMap& params,
