    ...
    onNext: {
        ...
        if (paymentByCashPage.hasDebt || paymentByCashPage.hasCredit) {
            ...
            paymentByCashPage.nextPage.properties = {
                ...
                "canAcceptAlternatePaymentMethod": paymentByCashPage.cartModel.canAcceptCash || paymentByCashPage.cartModel.canAcceptCard,
                "isCashPayment": true,
                "totalCost": paymentByCashPage.cartModel.totalCost,
                "cartModel": paymentByCashPage.cartModel
            };
        } else if ((paymentByCashPage.cartModel.customerName.trim() === "" || paymentByCashPage.cartModel.customerPhoneNumber.trim() === "") && (paymentByCashPage.hasDebt || paymentByCashPage.hasCredit)) {
    ...
    contentItem: FocusScope {
        Column {
            ...
            FluidControls.SubheadingLabel {
                wrapMode: Text.Wrap
                text: {
                    qsTr("The total cost is <b>%1</b>. Please enter the amount paid by the customer below.").arg(Number(paymentByCashPage.totalCost).toLocaleCurrencyString(Qt.locale(GlobalSettings.currencyLocaleName)));
                }
            ...
            FluidControls.SubheadingLabel {
                wrapMode: Text.Wrap
                color: paymentByCashPage.hasDebt ? Stylesheet.debtorRed : paymentByCashPage.hasCredit ? Stylesheet.creditorYellow : Material.theme === Material.Dark ? Stylesheet.white : Stylesheet.black
                text: {
                    if (paymentByCashPage.hasDebt)
                        return qsTr("You owe the %1 ").arg(paymentByCashPage.reason === PaymentWizard.Sales ? "customer" : "vendor") + "<b>%2</b>.".arg(Number(Math.abs(paymentByCashPage.balance)).toLocaleCurrencyString(Qt.locale(GlobalSettings.currencyLocaleName)));
                    else if (paymentByCashPage.hasCredit)
                        return qsTr("The %1 owes you ").arg(paymentByCashPage.reason === PaymentWizard.Sales ? "customer" : "vendor") + "<b>%2</b>.".arg(Number(Math.abs(paymentByCashPage.balance)).toLocaleCurrencyString(Qt.locale(GlobalSettings.currencyLocaleName)));
