...
#include "qmlapi/debtor/qmldebttransactionmodel.h"
...
{
    QSignalSpy imageSourceChangedSpy(m_debtTransactionModel,
                                     &QMLDebtTransactionModel::imageUrlChanged);
    ...
    QCOMPARE(m_debtTransactionModel->imageUrl(),
             QStringLiteral("image/source"));
    QCOMPARE(imageSourceChangedSpy.count(), 1);
    imageSourceChangedSpy.clear();

    // STEP: Ensure user is not notified if image source is set to the same
    // value.
    m_debtTransactionModel->setImageUrl(QStringLiteral("image/source"));
...
{
    QSignalSpy firstNameChangedSpy(m_debtTransactionModel,
                                   &QMLDebtTransactionModel::firstNameChanged);
...
{
    QSignalSpy lastNameChangedSpy(m_debtTransactionModel,
                                  &QMLDebtTransactionModel::lastNameChanged);
...
{
    QSignalSpy preferredNameChangedSpy(
        m_debtTransactionModel, &QMLDebtTransactionModel::preferredNameChanged);
    ...
    QCOMPARE(m_debtTransactionModel->preferredName(),
             QStringLiteral("Preferred"));
    QCOMPARE(preferredNameChangedSpy.count(), 1);
    preferredNameChangedSpy.clear();

    // STEP: Ensure user is not notified if preferred name is set to the same
    // value.
    m_debtTransactionModel->setPreferredName(QStringLiteral("Preferred"));
...
{
    QSignalSpy phoneNumberChanged(m_debtTransactionModel,
                                  &QMLDebtTransactionModel::phoneNumberChanged);
    ...
    QCOMPARE(m_debtTransactionModel->phoneNumber(),
             QStringLiteral("123456789"));
    QCOMPARE(phoneNumberChanged.count(), 1);
    phoneNumberChanged.clear();

    // STEP: Ensure user is not notified if primary phone number is set to the
    // same value.
    m_debtTransactionModel->setPhoneNumber(QStringLiteral("123456789"));
...
{
    QSignalSpy noteChangedSpy(m_debtTransactionModel,
                              &QMLDebtTransactionModel::noteChanged);
...
{
    QSignalSpy rowsInsertedSpy(m_debtTransactionModel,
                               &QMLDebtTransactionModel::rowsInserted);
    ...
    QCOMPARE(m_debtTransactionModel->index(0)
                 .data(QMLDebtTransactionModel::TransactionIdRole)
                 .toInt(),
             0);
    QCOMPARE(m_debtTransactionModel->index(0)
                 .data(QMLDebtTransactionModel::RelatedTransactionIdRole)
                 .toInt(),
             0);
    QCOMPARE(m_debtTransactionModel->index(0)
                 .data(QMLDebtTransactionModel::RelatedTransactionRole)
                 .toString(),
             QString());
    QCOMPARE(m_debtTransactionModel->index(0)
                 .data(QMLDebtTransactionModel::DueDateRole)
                 .toDateTime(),
             currentDateTime);
    QCOMPARE(m_debtTransactionModel->index(0)
                 .data(QMLDebtTransactionModel::NoteRole)
                 .toString(),
             QStringLiteral("Note"));
    QCOMPARE(m_debtTransactionModel->index(0)
                 .data(QMLDebtTransactionModel::TotalDebtRole)
                 .toDouble(),
             1234.56);

    // STEP: Ensure debt payment model is updated properly.
    //    DebtPaymentModel *debtPaymentModel =
    //    m_debtTransactionModel->index(0).data(QMLDebtTransactionModel::PaymentModelRole).value<DebtPaymentModel
    //    *>(); QVERIFY(debtPaymentModel != nullptr);
    //    QCOMPARE(debtPaymentModel->rowCount(), 1);
    //    QCOMPARE(debtPaymentModel->index(0).data(DebtPaymentModel::AmountPaidRole).toDouble(),
    //    0.0);
    //    QCOMPARE(debtPaymentModel->index(0).data(DebtPaymentModel::NoteRole).toString(),
    //    QString());
    //    QCOMPARE(debtPaymentModel->index(0).data(DebtPaymentModel::ArchivedRole).toDouble(),
    //    0.0);
}

void QMLDebtTransactionModelTest::testUpdateDebt()
{
    QSignalSpy dataChangedSpy(m_debtTransactionModel,
                              &QMLDebtTransactionModel::dataChanged);
    const QDateTime& currentDateTime = QDateTime::currentDateTime();
    ...
    //    m_debtTransactionModel->updateDebt(0, currentDateTime.addDays(1),
    //    QStringLiteral("Note2"));

    //    // STEP: Ensure model is updated properly.
    //    QCOMPARE(dataChangedSpy.count(), 1);
    //    QCOMPARE(m_debtTransactionModel->rowCount(), 1);
    //    QCOMPARE(m_debtTransactionModel->index(0).data(QMLDebtTransactionModel::DueDateRole).toDateTime(),
    //    currentDateTime.addDays(1));
    //    QCOMPARE(m_debtTransactionModel->index(0).data(QMLDebtTransactionModel::NoteRole).toString(),
    //    QStringLiteral("Note2"));
    ...
    //    QCOMPARE(debtPaymentModel->index(0).data(DebtPaymentModel::NoteRole).toString(),
    //    QString());
    //    QCOMPARE(debtPaymentModel->index(0).data(DebtPaymentModel::ArchivedRole).toBool(),
    //    false);
    //    QCOMPARE(debtPaymentModel->index(0).data(DebtPaymentModel::CreatedRole).toDateTime(),
    //    QDateTime::currentDateTime());
}

void QMLDebtTransactionModelTest::testRemoveDebt()
{
    QSignalSpy rowsRemovedSpy(m_debtTransactionModel,
                              &QMLDebtTransactionModel::rowsRemoved);
    const QDateTime& currentDateTime = QDateTime::currentDateTime();
...
void QMLDebtTransactionModelTest::testAddPayment()
{
    ...
    //    QSignalSpy rowsInsertedSpy(debtPaymentModel,
    //    &DebtPaymentModel::rowsInserted);

    //    // STEP: Add payment.
    //    m_debtTransactionModel->addPayment(0, 12.34, QStringLiteral("Payment
    //    note")); QCOMPARE(rowsInsertedSpy.count(), 1);
    //    QCOMPARE(debtPaymentModel->index(1).data(DebtPaymentModel::AmountPaidRole).toDouble(),
    //    12.34);
    //    QCOMPARE(debtPaymentModel->index(1).data(DebtPaymentModel::NoteRole).toString(),
    //    QStringLiteral("Payment note"));
    //    QCOMPARE(debtPaymentModel->index(1).data(DebtPaymentModel::ArchivedRole).toBool(),
    //    false);
    //    QCOMPARE(debtPaymentModel->index(1).data(DebtPaymentModel::CreatedRole).toDateTime(),
    //    QDateTime::currentDateTime());
}
...
void QMLDebtTransactionModelTest::testUpdatePayment()
{
    ...
    //    QSignalSpy dataChangedSpy(m_debtTransactionModel,
    //    &DebtPaymentModel::dataChanged);

    //    // STEP: Update payment.
    //    m_debtTransactionModel->updatePayment(0, 0, 12.34,
    //    QStringLiteral("Payment note")); QCOMPARE(dataChangedSpy.count(), 1);
    //    QCOMPARE(debtPaymentModel->index(0).data(DebtPaymentModel::AmountPaidRole).toDouble(),
    //    12.34);
    //    QCOMPARE(debtPaymentModel->index(0).data(DebtPaymentModel::NoteRole).toString(),
    //    QStringLiteral("Payment note"));
...
void QMLDebtTransactionModelTest::testRemovePayment()
{
    ...
    //            *>();
    //    QSignalSpy rowsRemovedSpy(debtPaymentModel,
    //    &DebtPaymentModel::rowsRemoved);
...
void QMLDebtTransactionModelTest::testSetDebtorId()
{
    ...
    QSignalSpy debtorIdChangedSpy(m_debtTransactionModel,
                                  &QMLDebtTransactionModel::debtorIdChanged);
    QSignalSpy successSpy(m_debtTransactionModel,
                          &QMLDebtTransactionModel::success);
...
void QMLDebtTransactionModelTest::testSubmitDebt()
{
    auto databaseWillReturnSingleDebt = [this]() {
        ...
        const QVariantList paymentGroups{
            QVariantMap{{"debt_transaction_id", 1},
                        {"debt_payment_id", 1},
        ...
        m_thread->result().setOutcome(QVariantMap{
            ...
            {"primary_phone_number",
             request.params().value("primary_phone_number")},
            {"note", ""},
    ...
    QCOMPARE(successSpy.count(), 1);
    QCOMPARE(successSpy.takeFirst().first().value<ModelResult>().code(),
             QMLDebtTransactionModel::AddDebtorSuccess);
    QCOMPARE(m_debtTransactionModel->rowCount(), 0);
}
...
void QMLDebtTransactionModelTest::testSubmitPayment()
{
    ...
    // STEP: Add debt.
    m_debtTransactionModel->addDebt(1234.56, dueDate, QStringLiteral("Note"));
    //    m_debtTransactionModel->addPayment(0, 1234.56, QStringLiteral("Payment
    //    Note"));
    ...
    //    QVERIFY(m_debtTransactionModel->submit());
    //    QCOMPARE(successSpy.count(), 1);
    //    QCOMPARE(successSpy.takeFirst().first().value<QMLDebtTransactionModel::SuccessCode>(),
    //    QMLDebtTransactionModel::AddDebtorSuccess);
    //    QCOMPARE(m_debtTransactionModel->rowCount(), 0);
}
