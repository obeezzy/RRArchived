...
void QMLClientModelTest::testModel()
{
    QAbstractItemModelTester(
        m_clientModel, QAbstractItemModelTester::FailureReportingMode::Fatal,
        this);
}

void QMLClientModelTest::testViewClients()
{
    const QVariantMap client{{"client_id", 1},
    ...
    auto databaseWillReturn = [this](const QVariantList& clients) {
        ...
        m_thread->result().setOutcome(
            QVariantMap{{"clients", clients}, {"record_count", 1}});
    };
    ...
    QCOMPARE(m_clientModel->index(0)
                 .data(QMLClientModel::PreferredNameRole)
                 .toString(),
             client["preferred_name"].toString());
    QCOMPARE(m_clientModel->index(0)
                 .data(QMLClientModel::PhoneNumberRole)
                 .toString(),
...
{
    QVariantMap client1{{"client_id", 1},
                        {"preferred_name", "Preferred"},
                        {"phone_number", "123456789"}};
    QVariantMap client2{{"client_id", 2},
                        {"preferred_name", "Preferred again"},
                        {"phone_number", "987654321"}};
    auto databaseWillReturn = [this](const QVariantList& clients) {
        m_thread->result().setSuccessful(true);
        m_thread->result().setOutcome(QVariantMap{
            {"clients", clients}, {"record_count", clients.count()}});
    };
